...
test('Swarm config has required structure', () => {
  ...
  // Support both old (specialistDomains) and new (managerDomains) format
  assert.ok(config.specialistDomains || config.managerDomains, 'Should have domains');
});
...
test('Swarm config has valid domains', () => {
  ...
  const domainsObj = config.specialistDomains || config.managerDomains;
  const domains = Object.keys(domainsObj);
  assert.ok(domains.length > 0, 'Should have at least one domain');

  for (const domain of domains) {
    const domainConfig = domainsObj[domain];
    // New format has 'manager' and 'ics', old format has 'focus'
    assert.ok(domainConfig.focus || domainConfig.manager || domainConfig.ics,
      `${domain} should have focus/manager/ics`);
  }
});
